...
TOR @ LAA | April 22, 2026 | 3:07 PM ET
CONFIRMED LINEUPS
"""
...
HOME = 'LAA'
AWAY_SP = 'Eric Lauer'
HOME_SP = 'Jose Soriano'
AWAY_SP_ERA = 7.13
HOME_SP_ERA = 0.28

# CONFIRMED LINEUPS (name, hand)
AWAY_LINEUP = [
    ('Nathan Lukes', 'L'), ('E. Clement', 'R'), ('V. Guerrero', 'R'),
    ('J. Sanchez', 'L'), ('Lenyn Sosa', 'R'), ('D. Varsho', 'L'),
    ('K. Okamoto', 'R'), ('A. Gimenez', 'L'), ('T. Heineman', 'S'),
]
HOME_LINEUP = [
    ('Zach Neto', 'R'), ('Mike Trout', 'R'), ('Jo Adell', 'R'),
    ('Jorge Soler', 'R'), ('O. Peraza', 'R'), ('N. Schanuel', 'L'),
    ('V. Grissom', 'R'), ('L. O\'Hoppe', 'R'), ('B. Teodosio', 'R'),
]
...
PARK_FACTOR = 99  # Angel Stadium
IS_DOME = False
TEMP_F = 72
WIND_MPH = 6
WIND_ANGLE = 90  

# Vegas (not in screenshot)
...
def main():
    ...
    print(f"║        {AWAY} @ {HOME} | {datetime.datetime.now().strftime('%B %d, %Y')} | 3:07 PM ET           ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    ...
    print(f"  Venue:        Angel Stadium | {TEMP_F}°F | Wind {WIND_MPH} mph")
    ...
    if PARK_FACTOR > 100:
        print(f"  ⚠ Angel Stadium is slightly hitter-friendly (PF {PARK_FACTOR})")
    ...
    print(f"  ℹ {AWAY} has {away_lefties}/9 LHB/SHB vs RHP {HOME_SP}")
