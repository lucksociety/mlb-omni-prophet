...
NYY @ BOS | April 22, 2026 | 6:45 PM ET
...
HOME = 'BOS'
AWAY_SP = 'Max Fried'
HOME_SP = 'Ranger Suarez'
AWAY_SP_ERA = 2.97
HOME_SP_ERA = 3.22

# LINEUPS (name, hand)
AWAY_LINEUP = [
    ('Ben Rice', 'L'), ('Aaron Judge', 'R'), ('C. Bellinger', 'L'),
    ('G. Stanton', 'R'), ('T. Grisham', 'L'), ('Amed Rosario', 'R'),
    ('J. Chisholm', 'L'), ('J. Caballero', 'R'), ('Austin Wells', 'L'),
]
HOME_LINEUP = [
    ('C. Rafaela', 'R'), ('W. Contreras', 'R'), ('Wilyer Abreu', 'L'),
    ('Trevor Story', 'R'), ('A. Monasterio', 'R'), ('Jarren Duran', 'L'),
    ('Caleb Durbin', 'R'), ('Connor Wong', 'R'), ('I. Kiner-Falefa', 'R'),
]
...
def main():
    ...
    print(f"║        {AWAY} @ {HOME} | {datetime.datetime.now().strftime('%B %d, %Y')} | 6:45 PM ET           ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    
    print(f"\n── 1. DATA VERIFICATION ─────────────────────────────────────────")
    print(f"  Lineups:      ● NYY (Expected) | BOS (Confirmed)")
    print(f"  Starters:     ● CONFIRMED")
    print(f"  Umpire:       Neutral")
    print(f"  Venue:        Fenway Park | {TEMP_F}°F | Wind {WIND_MPH} mph")
    print(f"  Park Factor:  {PARK_FACTOR} (hitter-friendly)")
    ...
    if PARK_FACTOR > 100:
        print(f"  ⚠ Fenway Park is hitter-friendly (PF {PARK_FACTOR})")
    ...
    print(f"  ℹ {HOME} has {home_lefties}/9 LHB/SHB vs LHP {AWAY_SP}")
