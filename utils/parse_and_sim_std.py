...
def stochastic_event_count(lam):
    ...
    # This core sampler generates individual event outcomes for the 100k-iteration sim.
    if lam > 500:
        # Approximate with Normal for large lambda to avoid exp(-500) underflow
        val = int(random.gauss(lam, math.sqrt(lam)))
        return max(0, val)
    L = math.exp(-lam)
    k = 0
    p_val = 1.0
...
def nbinom_rvs(n, p):
    ...
    return stochastic_event_count(lam)
...
def run_simulation(
    ...
    away_expected_runs = max(1.5, min(away_expected_runs, 12.0))
    home_expected_runs = max(1.5, min(home_expected_runs, 12.0))

    k_disp = 1.2
    away_p = k_disp / (k_disp + away_expected_runs)
    home_p = k_disp / (k_disp + home_expected_runs)

    away_runs = [nbinom_rvs(k_disp, away_p) for _ in range(n_sims)]
    home_runs = [nbinom_rvs(k_disp, home_p) for _ in range(n_sims)]
...
def calc_percentile(arr, p):
    ...
    idx = int(len(sorted_arr) * (p / 100.0))
...
def analyze_results(away_runs, home_runs, away_mean, home_mean, game_data):
    ...
    away_wins = sum(1 for i in range(n) if away_runs[i] > home_runs[i])
    home_wins = sum(1 for i in range(n) if home_runs[i] > away_runs[i])
    ties = n - away_wins - home_wins
    away_win_pct = (away_wins + ties * 0.48) / n * 100
    home_win_pct = (home_wins + ties * 0.52) / n * 100

    pitchers_duel = sum(1 for t in total_runs if t <= 3) / n * 100
    slugfest = sum(1 for t in total_runs if t >= 12) / n * 100
    low_scoring = sum(1 for t in total_runs if 4 <= t <= 6) / n * 100
    moderate_game = sum(1 for t in total_runs if 7 <= t <= 9) / n * 100

    total_std = calc_std(total_runs)
    p25 = calc_percentile(total_runs, 25)
    p75 = calc_percentile(total_runs, 75)

    run_line_away = sum(1 for i in range(n) if away_runs[i] - home_runs[i] > 1.5) / n * 100
    run_line_home = sum(1 for i in range(n) if home_runs[i] - away_runs[i] > 1.5) / n * 100

    ou_totals = {}
    for total in [6.5, 7.0, 7.5, 8.0, 8.5, 9.0, 9.5, 10.0]:
        ou_totals[total] = sum(1 for t in total_runs if t > total) / n * 100
        ...
        for row in reader:
            ...
            if current_section:
                if first_val == '#' and not headers:
                    headers = [h.split()[0] if h.strip() else f"Col_{i}" for i, h in enumerate(row)] # Take first word of header to simplify
                    continue
