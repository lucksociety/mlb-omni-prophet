...
# V2.0 Integration: K-Prophet Strikeout Engine
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "models/K Prophet"))
from engine import KProphetEngine
from k_pipeline import KProphetMaster # Ensure master logic is available
...
class Prediction:
    ...
    def __str__(self) -> str:
        ...
        # Calculate 1st Inning K Projections for reporting
        top_k_prob = (1.0 - self.top_nr_prob) * 1.5 # Heuristic for K-dominance reporting
        bot_k_prob = (1.0 - self.bottom_nr_prob) * 1.5
        ...
        return (
            ...
            f"{line}\n"
            f"  1ST INNING K-PROPHET PROFILE:\n"
            f"    {g.home_pitcher:16} : {'HIGH' if self.top_nr_prob > 0.8 else 'MID' if self.top_nr_prob > 0.7 else 'LOW'} Strikeout Threat\n"
            f"    {g.away_pitcher:16} : {'HIGH' if self.bottom_nr_prob > 0.8 else 'MID' if self.bottom_nr_prob > 0.7 else 'LOW'} Strikeout Threat\n"
            f"{line}\n"
            f"  Breakdown:\n"
...
class FirstInningSim:
    def __init__(self, pitcher_name: str, lineup_names: List[str], team_name: str, ballpark: str, env_factor: float = 1.0, umpire_zone: str = 'neutral'):
        ...
        self.umpire_zone = umpire_zone
        
        # Integration: K-Prophet Engine for physics-based K projections
    ...
    def calculate_outcome_probs(self, batter: Dict[str, Any]) -> Dict[str, float]:
        ...
        # V20 SYNC: Pull synchronized Umpire Bias from Hub or use passed zone
        ump_zone = self.umpire_zone
        
        env = {
            "Weather": {"temp": 72}, # Standardized for NRFI unless live data available
            "Umpire": {"CS_pct": 16.5, "zone_type": ump_zone},
            "ParkFactor": self.park_factor,
            "is_1st_inning": True
        }
        
        # Use K-Prophet's solve_physics_pk for elite accuracy
        prob_k = self.k_engine.solve_physics_pk(self.p_stats, batter, env)
        ...
        adj_babip = base_babip * wrc_mod * xera_mod * self.park_factor * self.env_factor * PITCHER_SETTLING_IN_MOD
        
        prob_in_play = 1.0 - prob_k - prob_bb
        prob_hit = prob_in_play * adj_babip
        prob_out = prob_in_play * (1.0 - adj_babip)
        
        # V20 SYNC: Use precision HR probabilities from HR-Alpha if available
        batter_name = batter.get("Name")
        hr_prob_per_pa = HUB.get(f"HR_Prob_{self.p_name}_{batter_name}")
        
        if hr_prob_per_pa:
            # HR-Alpha returns probability per game (usually ~4 PA), so we scale to per-PA
            # (Heuristic: HR_per_PA = Prob_Game / 4.1)
            scaled_hr_prob = hr_prob_per_pa / 4.1
            # Adjust the hit share logic to match this precision value
            hr_share = scaled_hr_prob / prob_hit if prob_hit > 0 else 0.13
            hr_share = max(0.01, min(0.40, hr_share))
        else:
            # V3.1: Dynamic Hit Distribution (Fallback)
    ...
    def simulate_half_inning(self, n_sims: int = 100000) -> float:
...
def predict_game(game: GameInput, env_factor: float = 1.0, umpire_zone: str = 'neutral') -> Prediction:
    notes: List[str] = []
    
    # 1. Sim Top 1st (Away Batters vs Home Pitcher)
    top_sim = FirstInningSim(game.home_pitcher, game.away_lineup, game.away_team, game.ballpark, env_factor=env_factor, umpire_zone=umpire_zone)
    top_nr_prob = top_sim.simulate_half_inning()
    
    # 2. Sim Bottom 1st (Home Batters vs Away Pitcher)
    bottom_sim = FirstInningSim(game.away_pitcher, game.home_lineup, game.home_team, game.ballpark, env_factor=env_factor, umpire_zone=umpire_zone)
...
if __name__ == "__main__":
    ...
    if os.path.exists(games_data_path):
        try:
            ...
            sys.path.append(os.path.join(os.path.dirname(__file__), "models/K Prophet"))
            ...
            print(f"Error loading games_data.py: {e}")
            # Fallback
