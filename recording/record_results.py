...
def update_predictions_csv(row_index, away_score, home_score):
    ...
    if row_index >= len(rows):
        print(f"  [ERROR] Row {row_index} not found.")
        return False
...
def update_audit_json(away_team, home_team, date_str, away_score=None, home_score=None,
                      sp_name=None, sp_k=None):
    ...
    for filepath in files:
        ...
        if away_score is not None and home_score is not None:
            ...
            audit['results']['winner'] = away_team if away_score > home_score else home_team
            audit['results']['status'] = 'COMPLETE'

        if sp_name and sp_k is not None:
            # Check if this name is the away or home SP
            if sp_name == audit['inputs'].get('away_sp', {}).get('name'):
                audit['results']['away_sp_k_actual'] = sp_k
            elif sp_name == audit['inputs'].get('home_sp', {}).get('name'):
                audit['results']['home_sp_k_actual'] = sp_k

        # Recalculate accuracy if results are partially complete
        if 'accuracy' not in audit:
            audit['accuracy'] = {}

        if audit['results'].get('status') == 'COMPLETE':
            score_proj = audit.get('outputs', {}).get('score_projection', {})
            if score_proj:
                pred_away = score_proj.get('away_mu', 0)
                pred_home = score_proj.get('home_mu', 0)
                audit['accuracy']['away_error'] = audit['results']['away_score'] - pred_away
                audit['accuracy']['home_error'] = audit['results']['home_score'] - pred_home
                audit['accuracy']['total_error'] = audit['results']['total'] - (pred_away + pred_home)
                audit['accuracy']['ml_correct'] = (pred_away > pred_home) == (audit['results']['away_score'] > audit['results']['home_score'])

        if audit['results'].get('away_sp_k_actual') is not None:
            pred_k = audit.get('outputs', {}).get('k_projections', {}).get('away', {}).get('final_k', 0)
            audit['accuracy']['away_k_error'] = audit['results']['away_sp_k_actual'] - pred_k
        if audit['results'].get('home_sp_k_actual') is not None:
            pred_k = audit.get('outputs', {}).get('k_projections', {}).get('home', {}).get('final_k', 0)
            audit['accuracy']['home_k_error'] = audit['results']['home_sp_k_actual'] - pred_k
        ...
        print(f"  [OK] Updated audit: {os.path.basename(filepath)}")
    ...
    if pending_games:
        ...
        for p in pending_games:
            ...
            try:
                ...
                print(f"    [OK] Recorded: {away} {away_score} — {home} {home_score}")
            except (ValueError, IndexError):
                print(f"    [ERROR] Invalid input. Skipping.")
    else:
        ...
        for p in pending_ks:
            ...
            try:
                ...
                if bet and bet.lower() != 's':
                    ...
                    print(f"    [RESULT] {result} ({pnl:+.2f} units)")
                ...
                if len(parts) == 2:
                    ...
                    update_audit_json(away_t, home_t, date_str, 
                                      sp_name=name, sp_k=actual_k)

                print(f"    [OK] Recorded: {name} = {actual_k} K's")
            except ValueError:
                print(f"    [ERROR] Invalid input. Skipping.")
