...
BASE_DIR = ROOT_DIR
PREDICTIONS_CSV = os.path.join(ROOT_DIR, 'data', 'predictions_history.csv')
K_TRACKER_CSV = os.path.join(ROOT_DIR, 'models', 'K Prophet', 'performance_tracker.csv')
AUDITS_DIR = os.path.join(ROOT_DIR, 'output', 'audits')
INTELLIGENCE_DIR = os.path.join(ROOT_DIR, 'output', 'intelligence')
...
def record_simulation(game_data, sim_results, market_odds=None):
    ...
    print(f"\n-- DATA RECORDER ---------------------------------------------------------------")
    print(f"  [OK] predictions_history.csv - game score prediction appended")
    print(f"  [OK] performance_tracker.csv - K-prop predictions appended")
    print(f"  [OK] audits/{os.path.basename(audit_path)} - full audit trail saved")
    print(f"  [OK] intelligence/{os.path.basename(intel_path)} - intel report saved")
...
def _record_intelligence_report(game_data, results, date_str):
    ...
    if disaster_traps:
        for d in disaster_traps:
            lines.append(f"- [!] **DISASTER TRAP**: {d}")
