...
### Distribution Models: Poisson and Markov Chains
Inside the Monte Carlo engine, advanced mathematics dictate the simulation.
1.  **Poisson Distributions:** This is a statistical concept used to predict the probability of a given number of events happening in a fixed interval of time or space. In baseball, it is heavily used to predict run distributions and strikeout props. For example, if a pitcher averages 6.5 strikeouts per 9 innings, a Poisson distribution will calculate the exact probability that he strikes out exactly 5, exactly 6, exactly 7, etc., in today's game.
