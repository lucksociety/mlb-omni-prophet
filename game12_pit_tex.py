...
PIT @ TEX | April 22, 2026 | 8:05 PM ET
EXPECTED LINEUPS
"""
...
# LINEUPS (name, hand)
AWAY_LINEUP = [
    ('Oneil Cruz', 'L'), ('Brandon Lowe', 'L'), ('B. Reynolds', 'S'),
    ("Ryan O'Hearn", 'L'), ('Nick Yorke', 'R'), ('M. Ozuna', 'R'),
    ('S. Horwitz', 'L'), ('K. Griffin', 'R'), ('Joey Bart', 'R'),
]
HOME_LINEUP = [
    ('B. Nimmo', 'L'), ('Corey Seager', 'L'), ('Jake Burger', 'R'),
    ('Joc Pederson', 'L'), ('Josh Jung', 'R'), ('Evan Carter', 'L'),
    ('Danny Jansen', 'R'), ('Josh Smith', 'L'), ('E. Duran', 'R'),
]
...
PARK_FACTOR = 104  # Globe Life Field
IS_DOME = True
TEMP_F = 74
WIND_MPH = 0
WIND_ANGLE = 0  
...
def main():
    ...
    print(f"║        {AWAY} @ {HOME} | {datetime.datetime.now().strftime('%B %d, %Y')} | 8:05 PM ET           ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    ...
    print(f"  Venue:        Globe Life Field | Dome (Controlled Env)")
    ...
    if PARK_FACTOR > 100:
        print(f"  ⚠ Globe Life Field is hitter-friendly (PF {PARK_FACTOR})")
