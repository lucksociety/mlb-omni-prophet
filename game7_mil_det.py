...
MIL @ DET | April 22, 2026 | 6:40 PM ET
...
# LINEUPS (name, hand)
AWAY_LINEUP = [
    ('Sal Frelick', 'L'), ('W. Contreras', 'R'), ('Brice Turang', 'L'),
    ('Gary Sanchez', 'R'), ('Jake Bauers', 'L'), ('B. Lockridge', 'R'),
    ('G. Mitchell', 'L'), ('Joey Ortiz', 'R'), ('D. Hamilton', 'L'),
]
HOME_LINEUP = [
    ('K. McGonigle', 'L'), ('G. Torres', 'R'), ('Jahmai Jones', 'R'),
    ('Riley Greene', 'L'), ('S. Torkelson', 'R'), ('W. Perez', 'S'),
    ('K. Carpenter', 'L'), ('Javier Baez', 'R'), ('Jake Rogers', 'R'),
]
...
def main():
    ...
    print(f"║        {AWAY} @ {HOME} | {datetime.datetime.now().strftime('%B %d, %Y')} | 6:40 PM ET           ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    
    print(f"\n── 1. DATA VERIFICATION ─────────────────────────────────────────")
    print(f"  Lineups:      ● MIL (Expected) | DET (Confirmed)")
    print(f"  Starters:     ● CONFIRMED")
    print(f"  Umpire:       Neutral")
    print(f"  Venue:        Comerica Park | {TEMP_F}°F | Wind {WIND_MPH} mph")
    ...
    if PARK_FACTOR < 98:
        print(f"  ⚠ Comerica Park is slightly pitcher-friendly (PF {PARK_FACTOR})")
