...
def load_predictions_history(filepath=None):
    ...
    if filepath is None:
        filepath = os.path.join(BASE_DIR, 'predictions_history.csv')
...
def load_k_tracker(filepath=None):
    ...
    if filepath is None:
        filepath = os.path.join(BASE_DIR, 'K Prophet', 'performance_tracker.csv')
    if not os.path.exists(filepath):
        return []
    results = []
    with open(filepath, 'r') as f:
...
def classify_game_archetype(away_era, home_era):
    """V17: Classify game into archetype for context-aware calibration.
    Fix 1 from May 5 forensic audit: Stop using flat modifiers."""
...
# V17: Per-archetype calibration modifiers
ARCHETYPE_MODIFIERS = {
...
def run_calibration_check():
    ...
    if pitcher_intel['worst_misses']:
        ...
        for m in pitcher_intel['worst_misses'][:5]:
            print(f"    {m['name']:20s} | {m['date']} | Pred: {m['projection']:.1f} → Actual: {m['actual']} | Error: {m['error']:+.1f}")
