...
def calculate_yrfi_probability(away_sp_era, home_sp_era, away_top4_wrc, home_top4_wrc, env_factor):
    ...
    Uses a Poisson-based approximation for the Top and Bottom of the 1st.
    """
    ...
    # P(YRFI) = 1 - P(NRFI)
    # P(NRFI) = P(Away 0 runs) * P(Home 0 runs)
    p_nrfi = math.exp(-lambda_A1) * math.exp(-lambda_H1)
...
def simulate_game_v6_5(away_sp_stats, home_sp_stats, away_bp, home_bp, 
    ...
    for i in range(N_SIMS):
        ...
        if h_sp_runs >= 5 or est_ppi > 22:
            actual_h_ip = max(0.5, actual_h_ip - (h_sp_runs - 2)*0.8) # Aggressive Dynamic Hook
        elif h_sp_runs >= 3:
            actual_h_ip = max(1.0, actual_h_ip - (h_sp_runs - 2)*0.5)
        elif h_sp_runs <= 1:
        ...
        if a_sp_runs >= 5 or est_ppi > 22:
            actual_a_ip = max(0.5, actual_a_ip - (a_sp_runs - 2)*0.8) # Aggressive Dynamic Hook
        elif a_sp_runs >= 3:
            actual_a_ip = max(1.0, actual_a_ip - (a_sp_runs - 2)*0.5)
        elif a_sp_runs <= 1:
        ...
        # We assume 3.8 batters per inning as an average baseline
        a_k_prob = calculate_odds_ratio_k(away_sp_stats['K_pct'], home_lineup_k, 22.7, away_sp_stats.get('Stuff', 100))
        
        run_penalty_a = 1.0
        if a_sp_runs >= 5: run_penalty_a = 0.70
        elif a_sp_runs >= 3: run_penalty_a = 0.85
        
        # Apply Umpire and Run penalties to the probability before converting to mu
        a_k_prob = a_k_prob * umpire_k_mod * run_penalty_a
        
        # Mean K per sim iteration = Prob * Batters Faced
        # Est Batters Faced = IP * 3.8 + Runs (approx)
        est_bf_a = (actual_a_ip * 3.8) + a_sp_runs
        raw_a_k_mu = a_k_prob * est_bf_a
        away_sp_k_dist.append(poisson_rvs(raw_a_k_mu))
        
        h_k_prob = calculate_odds_ratio_k(home_sp_stats['K_pct'], away_lineup_k, 22.7, home_sp_stats.get('Stuff', 100))
        
        run_penalty_h = 1.0
        if h_sp_runs >= 5: run_penalty_h = 0.70
        elif h_sp_runs >= 3: run_penalty_h = 0.85
        
        h_k_prob = h_k_prob * umpire_k_mod * run_penalty_h
        
        est_bf_h = (actual_h_ip * 3.8) + h_sp_runs
        raw_h_k_mu = h_k_prob * est_bf_h
...
def calculate_odds_ratio_k(p_k_pct, l_k_pct, lg_avg_k=22.7, stuff_plus=100):
    ...
    # Combined odds
    combined_odds = p_factor * l_factor * avg_odds
...
def run_v6_6_protocol(away_team, home_team, away_sp_name, home_sp_name,
    ...
    sections_25 = parse_csv('MLB Stats 2025') if os.path.exists('MLB Stats 2025') else None
    sections_car = parse_csv('MLB Stats Career.csv') if os.path.exists('MLB Stats Career.csv') else None
    ...
    try:
        yrfi_pred = yrfi.predict_game(game_input)
    ...
    if vegas_total is not None:
        blended_total = raw_total_mu * 0.65 + vegas_total * 0.35
        ...
        print(f"  [V6.6 VEGAS ANCHOR] Raw total: {raw_total_mu:.2f} → Blended: {total_mu:.2f} (Vegas: {vegas_total})")
    else:
        away_mu = raw_away_mu
        home_mu = raw_home_mu
