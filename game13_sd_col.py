...
SD @ COL | April 22, 2026 | 8:40 PM ET
EXPECTED LINEUPS
"""
...
AWAY = 'SDP'
HOME = 'COL'
AWAY_SP = 'Walker Buehler'
HOME_SP = 'Tomoyuki Sugano'
AWAY_SP_ERA = 4.58
HOME_SP_ERA = 3.92
...
HOME_LINEUP = [
    ...
    ('T. Freeman', 'R'), ('TJ Rumfield', 'L'), ('T. Johnston', 'L'),
    ('E. Tovar', 'R'), ('Kyle Karros', 'R'), ('B. Doyle', 'R'),
]
...
def main():
    ...
    print(f"║        SD @ COL | {datetime.datetime.now().strftime('%B %d, %Y')} | 8:40 PM ET           ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    ...
    print(f"  Park Factor:  {PARK_FACTOR} (extremely hitter-friendly)")
    ...
    print(f"\n── 3. LINEUP STRENGTH ───────────────────────────────────────────")
    print(f"  SD Expected wRC+: {away_wrc:.1f}")
    for name, hand in AWAY_LINEUP:
        print(f"    {hand} {name}")
    print(f"\n  COL Expected wRC+: {home_wrc:.1f}")
    for name, hand in HOME_LINEUP:
        print(f"    {hand} {name}")
    
    print(f"\n  Bullpen xFIP: SD {away_bp:.2f} | COL {home_bp:.2f}")
    
    print(f"\n── 4. MATHEMATICAL EXPECTANCY ({N_SIMS:,} SIMS) ─────────────────────")
    print(f"  ┌──────────────────────────────────────────────┐")
    print(f"  │  SD: {away_mu:.3f} runs  |  COL: {home_mu:.3f} runs   │")
    print(f"  │  PROJECTED TOTAL: {away_mu+home_mu:.3f} runs              │")
    print(f"  └──────────────────────────────────────────────┘")
    
    print(f"\n── 5. SCORE DISTRIBUTION ────────────────────────────────────────")
    print(f"  MOST LIKELY FINAL:  SD {mode[0]} — COL {mode[1]}  ({mode_ct/n*100:.1f}%)")
    for s, c in scores.most_common(6)[1:]:
        print(f"                      SD {s[0]} — COL {s[1]}  ({c/n*100:.1f}%)")
    
    print(f"\n── 6. WIN PROBABILITY ───────────────────────────────────────────")
    fav = "SD" if apct > hpct else HOME
    dog = HOME if fav == "SD" else "SD"
    fav_pct = max(apct, hpct)
    dog_pct = min(apct, hpct)
    print(f"  SD: {apct:.1f}%  |  COL: {hpct:.1f}%")
    ...
    if VEGAS_HOME_LINE:
        ...
        print(f"\n  Vegas Implied:  COL {vegas_home_pct:.1f}%")
        print(f"  Model:          COL {hpct:.1f}%")
        if abs(model_diff) > 3:
            direction = "OVER" if model_diff > 0 else "UNDER"
            print(f"  ⚡ MODEL {direction}VALUES COL by {abs(model_diff):.1f}pp vs Vegas")
        else:
    ...
    print(f"\n  Run Line: SD −1.5 → {rl_away:.1f}%  |  COL −1.5 → {rl_home:.1f}%")
    ...
    if PARK_FACTOR > 105:
        print(f"  ⚠ Coors Field effect active (PF {PARK_FACTOR}) → high expected scoring volatility")
    ...
    print(f"  ℹ SD has {away_lefties}/9 LHB/SHB vs RHP {HOME_SP}")
    print(f"  ℹ COL has {home_lefties}/9 LHB/SHB vs RHP {AWAY_SP}")
    
    print(f"\n{'='*66}")
    print(f"  FINAL PREDICTION:  SD {away_mu:.2f} — COL {home_mu:.2f}")
